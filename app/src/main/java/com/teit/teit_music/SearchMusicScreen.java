...
public class SearchMusicScreen extends Activity {
            ...
            public void run() {
                SearchMusicScreen.this.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (player.isPlaying()) {
                            int secall = (int) (Math.ceil(player.getCurrentPosition()) / 1000);
                            int min = secall / 60;
                            int sec = secall % 60;
    ...
    public void NextMusic(File[] files , ArrayList<String> nameFavoriteSongs){
        ...
        int position = 0;
        //todo : get position =>
        for (int starttick = 0; starttick < files.length; starttick++) {
            if (musicnameArray.get(0).equals(files[starttick].getName())) {
                position = starttick;
                break;
            }
        }
        ...
        if (music_logic == 0) {
            ...
            if (position == files.length - 1) {
                ...
                File newmusic = files[position + 1];
                NewMusic(nameFavoriteSongs,newmusic);
            }
        }
    }

    public void PreviousMusic(File[] files , ArrayList<String> nameFavoriteSongs){
        //set heart unfilled =>
        musicheart.setImageResource(R.drawable.heart);
    ...
    public void MusicEnded(File[] files , ArrayList<String> nameFavoriteSongs){
        //set heart unfilled =>
        musicheart.setImageResource(R.drawable.heart);
